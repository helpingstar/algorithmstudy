...
def bfs(r_x, r_y, b_x, b_y):
    ...
    while q:
        r_x, r_y, b_x, b_y, cnt, move = q.popleft()
        # print(f'[debug]  {(r_x, r_y, b_x, b_y, cnt)}')
        if cnt == 10:
            return -1
        for idx, func in enumerate([spin_up, spin_down, spin_left, spin_right]):
            ...
            if n_b_x == -1:
                continue
            if n_r_x == -1:
                return cnt+1, move+move_char[idx]
            q.append((n_r_x, n_r_y, n_b_x, n_b_y, cnt+1, move+move_char[idx]))
            visited.add((n_r_x, n_r_y, n_b_x, n_b_y))
            # print(f'[debug]: {(r_x, r_y, b_x, b_y)} -> {((n_r_x, n_r_y, n_b_x, n_b_y))}')
    return -1

ans = bfs(r_x, r_y, b_x, b_y)
if ans == -1:
    print(-1)
else:
    print(ans[0])
